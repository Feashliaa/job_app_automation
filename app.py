...
def insert_scraped_jobs(db, scraped_jobs, session_id, user_email, parsed_resume):
    ...
    for job in scraped_jobs:
        # Skip duplicates
        existing = (
            db.query(Job).filter_by(URL=job["URL"], user_email=user_email).first()
        )
        if existing:
            continue
...
def refresh_jobs():
    ...
    try:
        ...
        query = db.query(Job).filter(Job.user_email == user_email)

        if max_age_days is not None:
            try:
                days = int(max_age_days)
                cutoff_date = datetime.now().date() - timedelta(days=days)
                query = query.filter(Job.DateFound >= cutoff_date)
            except ValueError:
                pass  # ignore invalid max_age_days

        query = query.order_by(Job.DateFound.desc())
        ...
        # --- Apply pagination --- #
        jobs_in_db = query.offset((page - 1) * page_size).limit(page_size).all()
        jobs_list = [
            {
                "JobTitle": job.JobTitle,
                "Company": job.Company,
...
def limit_upload_size():
    if (
        ...
        and request.content_length > MAX_FILE_SIZE_MB * 1024 * 1024
    ):
...
def calculate_job_score(parsed_resume, job):
    ...
    resume_text = (
        " ".join(
            f"{category} {' '.join(skills)}" for category, skills in skills_dict.items()
        )
        + " "
    ...
    overlap_ratio = len(resume_words & job_words) / (
        len(job_words) or 1
    )  # overlap ratio
