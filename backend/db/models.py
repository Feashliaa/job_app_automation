...
class Job_Query(Base):
    ...
    # and set native_enum=False. This prevents storing the enum member
    # name (e.g. 'PastMonth').
    
    date_posted: Mapped[DatePosted] = mapped_column(SAEnum(DatePosted, 
    values_callable=lambda enum: [e.value for e in enum], 
    native_enum=False), default=DatePosted.PastWeek)
    
    experience_level: Mapped[ExperienceLevel] = mapped_column(SAEnum(ExperienceLevel, 
    values_callable=lambda enum: [e.value for e in enum], 
    native_enum=False), default=ExperienceLevel.EntryLevel)
...
class Scrape_Session(Base):
    ...
    status: Mapped[ScrapeStatus] = mapped_column(SAEnum(ScrapeStatus, 
    values_callable=lambda enum: [e.value for e in enum], 
    native_enum=False), default=ScrapeStatus.Running)
...
class Job(Base):
    ...
    values_callable=lambda enum: [e.value for e in enum],
    native_enum=False), default=JobStatus.New)
